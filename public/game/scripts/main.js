        ...
        constructor(iRuntime) {
            ...
            async function DoAsync(e) {
                ...
                if (e.action === "Signupemail" || e.action === "Signinemail" || e.action === "Signupname" || e.action === "Signinname") {
                    ...
                    firebase._LoginByEvent[e.sdkName] = 1;
                    var provider;
                    if (e.providerNo === 0) provider = new firebase.auth.GoogleAuthProvider();
                ...
                if (e.action === "init" || e.action === "reconnect") {
                    function setAuthListener() {
                        ...
                        if (firebase._loadedApp[e.sdkName] !== 1 || e.action === "reconnect") {
                            firebase.auth.onAuthStateChanged(myAuth, (user) => {
                                ...
                                if (user) {
                                    if (e.debug && firebase._AuthProEnabled[e.sdkName] === 1) {
                                        var debugProviders = "";
                                        for (var ij = 0; ij < authExpPro["countProviderPro"]; ij++) {

                                            debugProviders += authExpPro["providerSDPro"][ij]["providerId"];
                                            if (ij != authExpPro["countProviderPro"] - 1) debugProviders += ", ";

                                        }
                                        var explStg = "";
                                        if (firebase._LoginByEvent[e.sdkName] === 0) {
                                            _TrigNotEvent = 1;
                                            explStg = " --EXPLICIT";
                                        }
                                        else _TrigNotEvent = 0;
                                        firebase._LoginByEvent[e.sdkName] = 0;
                                        console.log("LOGGED IN" + explStg + "\nsdkObject: " + e.sdkName + "\nuserID: " + authExpBasic["myUID"] + "\nproviderCount: " + authExpPro["countProviderPro"] + "\nproviderList: " + debugProviders + "\nemail: " + authExpBasic["myEmail"] + "\nisEmailVerified: " + authExpBasic["isEmailVerified"] + "\nusername: " + authExpBasic["username"] + "\nphone: " + authExpBasic["phoneNo"] + "\nphotoURL: " + authExpBasic["photoURL"] + "\n ");
                                    }
                                    else if (e.debug) {
                    ...
                    if (e.action === "init") {
                        ...
                        if (e.sdkName === firebase.mainSdkName) {
                            ...
                            if (e.enable_appcheck) {
                                setScriptString(iAppCheck, "", "appcheck");
                            }
                            if (e.enable_auth) {
                                setScriptString(iAuth, e.remove_auth, "auth");
                            }
                            if (e.enable_database) {
                                setScriptString(iRD, e.remove_database, "database");
                            }
                            if (e.enable_storage) {
                                setScriptString(iStorage, e.remove_storage, "storage");
                            }
